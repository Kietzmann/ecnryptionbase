package edu.kytsmen.java.encryption.hashsums.core.sha3;

import edu.kytsmen.java.encryption.hashsums.core.sha3.utils.HexUtils;

import java.math.BigInteger;

import static edu.kytsmen.java.encryption.hashsums.core.sha3.utils.HexUtils.getReverseHex;
...
public class Keccak {
            ...
            for (int i = 0; i < 5; ++i) {
                for (int j = 0; j < 5; ++j) {
                    if ((5 * i + j) < (parameters.getR() / w)) {
                        Z += addZero(getReverseHex(S[j][i].toByteArray()), 16).substring(0, 16);
                    }
                }
            }

            doKeccackf(S);
        } while (Z.length() < parameters.getOutputLength() << 1);

        return Z.substring(0, parameters.getOutputLength() << 1);
    }
    ...
    private BigInteger[][] padding(String message, Parameters parameters) {
        int size;
        message += parameters.getD();

        while ((((message.length() / 2) << 3) % parameters.getR()) != ((parameters.getR() - 8))) {
            message += "00";
        }

        message += "80";
        size = (((message.length() / 2) << 3) / parameters.getR());
    ...
    private void initialize(int b) {
        w = b / 25;
        int l = (int) (StrictMath.log(w) / StrictMath.log(2));
        n = 12 + 2 * l;
    }
